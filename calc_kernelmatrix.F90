...
module calc_kernelmatrix
  ...
  &         calc_slowness_est_matrix_delaunay_shmdump
...
&                                            triangle_stationindex, nsta_count, tnbr)
  ...
  do j = 1, ntriangle
    ...
    !!find nadd_station additional stations based on the distance between grid and station
    if(nadd_station_aeluma .ge. 1) then
      allocate(add_station_distance(1 : nadd_station_aeluma), add_station_index(1 : nadd_station_aeluma))
      add_station_distance(1 : nadd_station_aeluma) = 1.0e+38
      add_station_index(1 : nadd_station_aeluma) = 0
      do jj = 1, nsta
        ...
        &                   distance = dist_tmp)
        do ii = 1, nadd_station_aeluma
          if(dist_tmp .le. add_station_distance(ii)) then
            do i = nadd_station_aeluma, ii + 1, -1
              add_station_distance(i) = add_station_distance(i - 1)
              add_station_index(i) = add_station_index(i - 1)
            enddo
...
subroutine calc_slowness_est_matrix_delaunay_shmdump(location_sta, station_winch, &
...
&                                                    triangle_stationindex, nsta_count, tnbr)
  ...
  do j = 1, ntriangle
    do i = 1, 3
      write(10, '(2(e15.7, 1x))') location_sta(station_winch(index_org(triangle_indices(i, j))))%lon, &
      &                           location_sta(station_winch(index_org(triangle_indices(i, j))))%lat
    enddo
    write(10, '(a)') ">"
  enddo
  close(10)
  ...
  &        triangle_stationindex(1 : 3 + nadd_station_aeluma, 1 : ntriangle))
  do j = 1, ntriangle
    used_station(1 : nsta) = .false.
    triangle_center(j)%x_east  = 0.0_fp
    triangle_center(j)%y_north = 0.0_fp
    ...
    do i = 1, 3
      used_station(index_org(triangle_indices(i, j))) = .true.
      triangle_center(j)%x_east = triangle_center(j)%x_east &
      &                         + location_sta(station_winch(index_org(triangle_indices(i, j))))%x_east
      triangle_center(j)%y_north = triangle_center(j)%y_north &
    ...
    if(nadd_station_aeluma .ge. 1) then
      ...
      do jj = 1, nsta
        if(is_usestation(jj) .eqv. .false.) cycle
        if(used_station(jj)  .eqv. .true. ) cycle
        call cartesian_dist(location_sta(station_winch(jj))%x_east,  triangle_center(j)%x_east,  &
        &                   location_sta(station_winch(jj))%y_north, triangle_center(j)%y_north, &
        &                   distance = dist_tmp)
        do ii = 1, nadd_station_aeluma
          if(dist_tmp .le. add_station_distance(ii)) then
            do i = nadd_station_aeluma, ii + 1, -1
