...
system "gmt set FONT_LABEL 10p,Helvetica";
...
foreach $index (@index_array){
  ...
  system "cat /dev/null | gmt psxy -JX1c -R0/1/0/1 -Sc0.1 -K -X3c -Y6c > $out";
  ...
  if (-f $coastline){
    system "gmt psxy $coastline -JX$size_x/$size_y -R$min_x/$max_x/$min_y/$max_y -W0.8p,black -O -K >> $out";
  }
  open OUT, " | gmt pstext -JX$size_x/$size_y -R0/$size_x/0/$size_y -N -F+f+a+j -Gwhite -O -K >> $out";
    if($simulation == 1){
      print OUT "$txt_x $txt_y 12p,Helvetica,black 0 LT Time: ${current_hh}hr ${current_mm}m\n";
    }else{
  ...
  system "gmt psscale -Dx$cpt_x/$cpt_y/$cpt_len/$cpt_width -B+l\"Geometrical spreading (10\@+-2\@+ km\@+-1\@+)\" \\
  ...
  system "gmt psscale -Dx$cpt_x/$cpt_y/$cpt_len/$cpt_width -B+l\"Radiation pattern (10\@+-2\@+ km\@+-1\@+)\" \\
                       -C$ampterm_cpt -O -K >> $out";
