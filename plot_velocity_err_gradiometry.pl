...
foreach $index (@index_array){
  ...
  if (-f $in2){
    open OUT, " | gmt psxy -JX$size_x/$size_y -R$min_x/$max_y/$min_y/$max_y -Ss$symbolsize -W0.5p,black \\
                           -C$cpt_vel -O -K -P >> $out";
    for($i = 0; $i <= $#x_east; $i++){
      ...
      #print stdout "$x_east[$i] $y_north[$i] $velocity_err\n";
    }
    close OUT;
  }
  ...
  system "gmt psscale -Dx$cpt_x/$cpt_y/$cpt_len/$cpt_width -B+l\"velocity error (m/s)\" -C$cpt_vel -O -K -P >> $out";
  ...
  system "gmt psscale -Dx$cpt_x/$cpt_y/$cpt_len/$cpt_width -B+l\"Azimuth error (degree)\" -C$cpt_az -O -K -P >> $out";
