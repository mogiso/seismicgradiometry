...
program seismicgradiometry_reducingvelocity
  ...
  do i = 1, nsta
    call tandem2(waveform_obs(:, i), waveform_obs(:, i), ntime, h, m, 1, uv)
  enddo
  deallocate(h, uv)
  ...
  do kk = 1, int(ntime / ntimestep)
    ...
    do jj = 1, ngrid_y
      do ii = 1, ngrid_x
        ...
        !correlation
        allocate(lagtime(1 : nsta_correlation(ii, jj)))
        k = 1
        do j = 1, nsta_count(ii, jj) - 1
          waveform_fft(1 : ntime_fft, 1) &
          &  = waveform_obs(timeindex - ntime_fft + 1 : timeindex, grid_stationindex(j, ii, jj))
          do i = j + 1, nsta_count(ii, jj)
            waveform_fft(1 : ntime_fft, 2) &
            &  = waveform_obs(timeindex - ntime_fft + 1 : timeindex, grid_stationindex(i, ii, jj))
        ...
        do i = 1, 2
          slowness(i, ii, jj) = slowness_xcorr(i, ii, jj) &
